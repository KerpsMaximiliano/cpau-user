...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.formregister = this.formBuilder.group({
      ...
      txtPass1: new FormControl("", [
        Validators.required,
        Validators.minLength(8),
      ]),
      txtPass2: new FormControl("", [
        Validators.required,
        Validators.minLength(8),
      ]),
    });
  ...
  onSubmit() {
    ...
    this.authenticationService.register(serializedForm).subscribe(
      (data) => {
  ...
  checkMatricula() {
    this.listConstantService
      .checkMatricula(
        this.f.sltTipoMatricula.value,
        this.f.txtNumeroMatricula.value
      )
      .subscribe(
        (data) => {
    ...
    if (this.f.cbMatriculado.value === true) {
      this.listConstantService
        .validMatricula(
          this.f.sltTipoMatricula.value,
          this.f.txtNumeroMatricula.value,
          this.f.sltTipoDoc.value,
          this.f.txtNumeroDoc.value
        )
        .subscribe(
          (data) => {
            if (data) {
